...
app.layout = html.Div(
    children=[
        html.Div(
            children=[
                html.H1(                    
                    children="Оценка качества арматуры по механическим свойствам", className="header-title"
                            ),
        ...
        html.Div(
            children=[
                html.Div(
                    children=[
                        html.Div(children="Выберите марку стали", className="menu-title"),
                        dcc.Dropdown(id='steelgrade_opts',
        ...
        html.Div(
            children=[
                dcc.Graph(
                    id='graph_gist',config={"displayModeBar": False}
                ),
            ],style = {'position': 'absolute',
                        'left': '10%',
                        'top': '40%',
        ...
        html.Div(
            ...
            html.Table(
                    ...
                    [html.Tr(
                        children=[
                        html.Th(['Характеристика'],style = {'border': '1px solid #000'}),
                        html.Th(['Критерий Пирсона'],style = {'border': '1px solid #000'}),
                        html.Th(['Критерий Колмогорова-Смирнова'],style = {'border': '1px solid #000'}),
                        html.Th(['Критерий Шапиро-Уилка'],style = {'border': '1px solid #000'}) ])]+
                    #Тело таблицы
                    [html.Tr([
                        html.Td('Предел текучести',style = {'border': '1px solid #000'}),
                     ...
                     html.Tr([
                        html.Td('Временное сопротивление',style = {'border': '1px solid #000'}),
                      ...
                      html.Tr([
                        html.Td('Относительное удлинение',style = {'border': '1px solid #000'}),
                       ...
                       html.Tr([
                        ...
                        html.Tr([
                        html.Td('Относительная площадь смятия поперечных рёбер',style = {'border': '1px solid #000'}),
...
def update_figure(grade,size):
    fig = make_subplots(rows=3, cols=2, subplot_titles=("Предел текучести, Н/м²",
    ...
    if (data_len < 25):
        ...
        fig.add_trace(go.Histogram(x=x_now,histnorm='probability',name='Предел текучести, Н/мм²',showlegend = False),  1, 1)

        loc, scale = norm.fit(x_now)
        n = norm(loc=loc, scale=scale)
        x_norm = np.arange(x_now.min(), x_now.max()+0.2, 0.2)
        fig.add_trace(go.Scatter(x=x_norm, y=5*n.pdf(x_norm), name='Теоретическая форма нормального распределения',line_color="red"),  1, 1)
        ...
        fig.add_trace(go.Histogram(x=x_now,histnorm='probability',name='Временное сопротивление, Н/мм²',showlegend = False),  1, 2)

        loc, scale = norm.fit(x_now)
        n = norm(loc=loc, scale=scale)
        x_norm = np.arange(x_now.min(), x_now.max()+0.2, 0.2)
        fig.add_trace(go.Scatter(x=x_norm, y=5*n.pdf(x_norm), name='Теоретическая форма нормального распределения',line_color="red",showlegend = False),  1, 2)
        ...
        fig.add_trace(go.Histogram(x=x_now,histnorm='probability',name='Относительное удлинение, %',showlegend = False),  2, 1)

        loc, scale = norm.fit(x_now)
        n = norm(loc=loc, scale=scale)
        x_norm = np.arange(x_now.min(), x_now.max()+0.2, 0.2)
        fig.add_trace(go.Scatter(x=x_norm, y=0.5*n.pdf(x_norm), name='Теоретическая форма нормального распределения',showlegend = False,line_color="red"),  2, 1)
        ...
        fig.add_trace(go.Histogram(x=x_now,histnorm='probability',name='Относительное равномерное удлинение, %',showlegend = False),  2, 2)

        loc, scale = norm.fit(x_now)
        n = norm(loc=loc, scale=scale)
        x_norm = np.arange(x_now.min(), x_now.max()+0.2, 0.2)
        fig.add_trace(go.Scatter(x=x_norm, y=0.25*n.pdf(x_norm), name='Теоретическая форма нормального распределения',showlegend = False,line_color="red"),  2, 2)
        ...
        #График 5
        x_now=df["fr"]
        fig.add_trace(go.Histogram(x=x_now,histnorm='probability',name='Относительная площадь смятия поперечных рёбер',showlegend = False),  3, 1)
    ...
    return s,fig, test_table[0,0],test_table[0,1],test_table[0,2],test_table[1,0],test_table[1,1],test_table[1,2],test_table[2,0],test_table[2,1],test_table[2,2],test_table[3,0],test_table[3,1],test_table[3,2],test_table[4,0],test_table[4,1],test_table[4,2]
